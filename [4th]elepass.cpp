...
void eBEND::Pass (vec &x)
{
    ...
    double lambda0 = lambda*C0;
    double lambda1 = lambda*C1;
    ...
    for(size_t i=0; i<Nint ;i++)
    {
        ...
        {
            //kick
            x[px_] += lambda1*iRHO*( x[ps_]*x[ps_]/(hs*hs) -dummy)/hs;
            x[s_] += x[ps_]/(hs*hs) *lambda1;
            x[vt_] += dummy *lambda1;
        }
        //transverse drift
        x[x_] += x[px_] *lambda1;
        ...
        if(dummy>1.0e-150){
            ...
            dummy=2.0*lambda1*(Lambda[1]*dummy2-Lambda[2]*x[Ss_]);
            x[Ss_]+=2.0*lambda1*(Lambda[2]*x[Sx_]-Lambda[1]*dummy2);
            x[Sx_]+=dummy;
        }
        //transverse drift
        x[x_] += x[px_] *lambda1;
        ...
        hs = (1.0 + x[x_]*iRHO );
        dummy = 1.0 +BETA2*x[dE_] -BETA2*log(hs); //electric potential
        x[px_] += (lambda1+lambda0)*iRHO*( x[ps_]*x[ps_]/(hs*hs) -dummy)/hs;
        x[s_] += x[ps_]/(hs*hs) *(lambda1+lambda0);
        x[vt_] += dummy *(lambda1+lambda0);
        //transverse drift
        x[x_] += x[px_] *lambda0;
        x[z_] += x[pz_] *lambda0;
        //spin kick
        ...
        if(dummy>1.0e-150){
            ...
            dummy=2.0*lambda0*(Lambda[1]*dummy2-Lambda[2]*x[Ss_]);
            x[Ss_]+=2.0*lambda0*(Lambda[2]*x[Sx_]-Lambda[1]*dummy2);
            x[Sx_]+=dummy;
        }
        //transverse drift
        x[x_] += x[px_] *lambda0;
        ...
        {
            x[px_] += 2.0*lambda1*iRHO*( x[ps_]*x[ps_]/(hs*hs) -dummy)/hs;
            x[s_] += x[ps_]/(hs*hs) *2.0*lambda1;
            x[vt_] += dummy *2.0*lambda1;
        }
    }
...
void eQUAD::Pass (vec &x)
{
    ...
    for(size_t i=0; i<Nint ;i++)
    {
        ...
        {
            //kick
            x[px_] -= dummy*K1*x[x_] *lambda1;
            x[pz_] += dummy*K1*x[z_] *lambda1;
            x[vt_] += dummy *lambda1;
        }
        //drift
        x[x_] += x[px_] *lambda1;
        ...
        if(dummy>1.0e-150){
            ...
            dummy=2.0*lambda1*(Lambda[1]*dummy2-Lambda[2]*x[Ss_]);
            x[Ss_]+=2.0*lambda1*(Lambda[2]*x[Sx_]-Lambda[0]*dummy2);
            x[Sx_]+=dummy;
        }
        //drift
        x[x_] += x[px_] *lambda1;
        ...
        x[px_] -= dummy*K1*x[x_] *(lambda1+lambda0);
        x[pz_] += dummy*K1*x[z_] *(lambda1+lambda0);
        x[vt_] += dummy *(lambda1+lambda0);
        //drift
        x[x_] += x[px_] *lambda0;
        x[z_] += x[pz_] *lambda0;
        ...
        if(dummy>1.0e-150){
            ...
            dummy=2.0*lambda0*(Lambda[1]*dummy2-Lambda[2]*x[Ss_]);
            x[Ss_]+=2.0*lambda0*(Lambda[2]*x[Sx_]-Lambda[0]*dummy2);
            x[Sx_]+=dummy;
        }
        //drift
        x[x_] += x[px_] *lambda0;
        ...
        {
            x[px_] -= dummy*K1*x[x_] *2.0*lambda1;
            x[pz_] += dummy*K1*x[z_] *2.0*lambda1;
            x[vt_] += dummy *2.0*lambda1;
        }
    }
    //fringe field kick
