...
Element::Element()
{
    ...
    R[0]= 0.78451361047755726382;
    R[1]= 0.23557321335935813368;
    R[2]=-1.17767998417887100695;
    R[3]= 1.31518632068391122204;
    for(size_t r=1;r<4;r++)  R[3+r]=R[3-r];
}
//=========================================================================
//                           electric elements
//=========================================================================
...
void eBEND::Pass (vec &x)
{
    ...
    for(size_t i=0; i<Nint ;i++)
    {
        ...
        if(i>Nint-3)
        {
            x[px_] += R[6]*lambda*iRHO*( x[ps_]*x[ps_]/(hs*hs) -dummy)/hs;
            x[s_] += x[ps_]/(hs*hs) *R[6]*lambda;
            x[vt_] += dummy *R[6]*lambda;
        }
        else
        {
            x[px_] += R[6]*2.0*lambda*iRHO*( x[ps_]*x[ps_]/(hs*hs) -dummy)/hs;
            x[s_] += x[ps_]/(hs*hs) *R[6]*2.0*lambda;
            x[vt_] += dummy *R[6]*2.0*lambda;
        }
    }
...
void eQUAD::Pass (vec &x)
{
    ...
    for(size_t i=0; i<Nint ;i++)
    {
        ...
        if(i==Nint-1)
        {
            x[px_] -= dummy*K1*x[x_] *R[6]*lambda;
            x[pz_] += dummy*K1*x[z_] *R[6]*lambda;
            x[vt_] += dummy *R[6]*lambda;
        }else
        {
            x[px_] -= dummy*K1*x[x_] *R[6]*2.0*lambda;
            x[pz_] += dummy*K1*x[z_] *R[6]*2.0*lambda;
            x[vt_] += dummy *R[6]*2.0*lambda;
        }
    }
    //fringe field kick
