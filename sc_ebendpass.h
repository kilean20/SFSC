            ...
            for (size_t npx=0;npx<4;npx++){
                for (size_t nz=0;nz<4;nz++){
                    for (size_t npz=0;npz<4;npz++){
                        if(nx+npx+nz+npz<5){
                            sigma(nx,npx,nz,npz)+=lambda*(nx*dx_eBend(nx,npx,nz,npz,tempSigma,r,iRho,Ksc,denom)
                                                         + npx*dpx_eBend(nx,npx,nz,npz,tempSigma,r,iRho,Ksc,denom)
                                                         + nz*dz_eBend(nx,npx,nz,npz,tempSigma,r,iRho,Ksc,denom)
                                                         + npz*dpz_eBend(nx,npx,nz,npz,tempSigma,r,iRho,Ksc,denom));
                        }
                    }
                }
            }
